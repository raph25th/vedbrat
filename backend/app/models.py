...
class CfaDeal(Base, TimestampMixin):
    ...
    client_id: Mapped[int] = mapped_column(ForeignKey("clients.id"), index=True)
    document_request_id: Mapped[int | None] = mapped_column(ForeignKey("document_issue_requests.id"), nullable=True, index=True)
    ...
    payment_received_amount: Mapped[Decimal | None] = mapped_column(Numeric(18, 2), nullable=True)
